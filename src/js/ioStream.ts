...
export class OutputStreamScreen {
  ...
  constructor() {
    ...
    this.root = document.querySelector(
      "#site-app .terminal-container .terminal-output"
    )!;
  ...
  createOutSection(): HTMLElementOutSection {
    const el = createHTMLElement("div", "", {
      class: "terminal-exe-section",
    }) as HTMLElementOutSection;
  ...
  printLink(param: OutputStreamJobParameters) {
    ...
    const link_name = createHTMLElement("p", param.name as string, {
      class: "name-text highlight text-label",
    });

    const link_sep = createHTMLElement("p", ":", { class: "sep" });

    const link_container = createHTMLElement("div", "", { class: "container" });
    const link_link = createHTMLElement("a", "", {
      class: "link",
      href: param.link as string,
      target: "_blank",
      rel: "noopener noreferrer",
    });
    const link_text = createHTMLElement("p", param.text as string, {
      class: "text clickable focus",
    });
  ...
  printCMDDesc(param: OutputStreamJobParameters) {
    ...
    const cmd_name = createHTMLElement("p", param.name as string, {
      class: "name focus clickable text-label",
    });
    const cmd_sep = createHTMLElement("p", ":", { class: "sep" });
    const cmd_desc = createHTMLElement("p", param.desc as string, {
      class: "desc",
    });
  ...
  printProject(param: OutputStreamJobParameters) {
    ...
    const pro_name = createHTMLElement("p", param.name as string, {
      class: "name focus text-label",
    });
    const pro_sep = createHTMLElement("p", ":", { class: "sep" });
    const pro_desc = createHTMLElement("p", param.desc as string, {
      class: "desc",
    });
    const pro_tags = createHTMLElement("p", param.tags as string, {
      class: "tags highlight",
    });
    ...
    for (let i = 0; i < (param.links as AnchorLink[]).length; i++) {
      ...
      pro_container.appendChild(
        createHTMLElement("a", link.title as string, {
          class: "link focus clickable",
          target: "_blank",
          href: link.link,
        })
      );
  ...
  printText(param: OutputStreamJobParameters) {
    const el = createHTMLElement(
      ...
      (param.class as string) !== undefined
        ? { class: param.class as string }
        : {}
    );
    this.append(el);
  }
  ...
  printSkills(param: OutputStreamJobParameters) {
    ...
    const skillsName = createHTMLElement("p", param.name as string, {
      class: "skills-name focus",
    });
    const skillsContainer = createHTMLElement("div", "", {
      class: "skills-container",
    });

    for (let i = 0; i < param.skills!.length; i++) {
      const skillEl = createHTMLElement("p", param.skills![i], {
        class: "item",
      });
      const skillContainer = createHTMLElement("div", "", {
  ...
  printTitle(param: OutputStreamJobParameters) {
    const el = createHTMLElement("p", param.text as string, {
  ...
  printPortfolio(param: OutputStreamJobParameters) {
    ...
    const containerLink = createHTMLElement("div", "", {
      class: "link-container",
    });

    const title = createHTMLElement("p", param.title as string, {
      class: "focus title",
    });
    const sum = createHTMLElement("p", param.sum as string, {
      class: "highlight sum",
    });
    const description = createHTMLElement("p", param.desc as string, {
      class: "desc",
    });
    ...
    (param.links as AnchorLink[]).forEach((l) => {
      const link = createHTMLElement(
        "a",
        '<span class="clickable">' + l.title + "</span>",
        { class: "highlight", target: "_blank", href: l.link }
      );

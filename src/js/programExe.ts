...
const PROGRAM_META = [
...

  {
    name: "semantic",
    func: semanticExe,
    desc: "display the semantic/no-script version of aghnu.me",
  },
...

  {
    name: "about",
    func: aboutExe,
    desc: "list info about this website/project",
  },
  {
    name: "resume",
    func: resumeExe,
    desc: "print the link to my current resume",
  },
  {
    name: "where",
    func: whereExe,
    desc: "print the place where I live and my current time",
  },
];
...
function updateLock(
  pid: string,
  message: string = "",
  input_func: ((cmd: string, param: ProgramExeParam) => void) | null = null
) {
...
function createDateStringElement(
  elClass: string,
  timeZone: string = "default"
) {
  // init element
  const dateString: HTMLParagraphElement = createHTMLElement("p", "", {
    class: "terminal-date" + " " + elClass,
  ...
  const updateTimeString = (el: HTMLParagraphElement) => {
    ...
    el.innerHTML =
      date.toLocaleDateString("en-CA", setting) +
      "&nbsp" +
      date.toLocaleTimeString("en-CA", setting);
  };
...
function semanticExe(
  param: ProgramExeParam,
  callback: (() => void) | null = null
) {
  const pid = genProcessID();
  lockSystem(
    pid,
    '<span class="highlight">[System is Currently Occupied]</span>'
  );

  const answer_yes = createHTMLElement("span", "YES", {
    class: "focus clickable",
  });
  const answer_no = createHTMLElement("span", "NO", {
    class: "focus clickable",
  });
      ...
      list: [
        {
          type: "text",
          parameters: {
            text: "Go to a text-based version of aghnu.me with reduced functionility and condensed information.",
          },
        },
        { type: "line", parameters: { height: 1 } },
        {
          type: "text",
          parameters: { text: "Do you wish to continue? Type (y/N)" },
        },
  ...
  updateLock(pid, "System is Currently Occupied", (cmd, param) => {
    if (cmd === "y" || cmd === "Y" || cmd === "yes") {
      ...
      param.outStream!.print({
        type: "text",
        parameters: {
          text: '<span class="highlight">[Pick your options to continue]</span>',
        },
      });
    }
  });
...
function skillsExe(
  ...
  for (let i = 0; i < skillsData.skills.length; i++) {
    ...
    skillsPrintJobs.push({
      type: "skills",
      parameters: { name: d.name, skills: d.skills },
    });
  ...
  param.outStream!.print({
    type: "list",
    parameters: {
      list: [...skillsPrintJobs],
      callback: callback!,
    },
  });
}

function portfolioExe(
  param: ProgramExeParam,
  callback: (() => void) | null = null
) {
  const portfolioProjectsPrintJobs: OutputStreamJob[] = [];
  for (let i = 0; i < portfolioData.projects.length; i++) {
    portfolioProjectsPrintJobs.push({
      type: "portfolio",
      parameters: portfolioData.projects[i],
    });
    portfolioProjectsPrintJobs.push({
      type: "line",
      parameters: { height: 1 },
  ...
  param.outStream!.print({
    type: "list",
    parameters: {
      list: [...portfolioProjectsPrintJobs],
      callback: callback!,
    },
  });
}
      ...
      list: [
        {
          type: "title",
          parameters: {
            text: GLOBAL_CONFIG.desktop_mode ? "D E V U E" : "Aghnu's Console",
          },
        },
        {
          type: "text",
          parameters: { text: "Gengyuan Huang's Homepage", class: "highlight" },
        },
        { type: "line", parameters: { height: 1 } },
      ],
    },
...
function cowsayExe(
  ...
  param.outStream!.print({
    ...
    parameters: {
      callback: callback!,
      list: [
        { type: "text", parameters: { text: text } },
        { type: "text", parameters: { text: "-".repeat(text.length) } },
        ...(() => {
  ...
  const sitemapPrintJobs: OutputStreamJob[] = ((): OutputStreamJob[] => {
    ...
    for (let i = 0; i < sitemapData.paths.length; i++) {
      ...
      list.push({
        type: "text",
        parameters: {
          text: `- <a target='_blank' class='clickable focus' href='${
            path.path
          }' >${sitemapData.origin + path.path}</a>`,
        },
      });
    }
    ...
    parameters: {
      list: [
        ...
        { type: "line", parameters: { height: 1 } },
        {
          type: "link",
          parameters: {
            link: "/static/doc/resume.pdf",
            name: "Resume",
            text: "resume_gengyuan.pdf",
            type: "link",
          },
        },
        { type: "line", parameters: { height: 1 } },
    ...
    parameters: {
      ...
      list: [
...

        {
          type: "text",
          parameters: {
            text: `Hello stranger! Welcome to my homepage. My name is <span class='highlight'>Gengyuan Huang</span>, a software developer...`,
          },
        },
        { type: "line", parameters: { height: 1 } },
        {
        ...
        { type: "separator", parameters: { height: 1 } },
        {
          type: "text",
          parameters: {
            text: "To navigate the site, you can either type commands into the console or click on the highlighted elements. Here are some useful commands:",
          },
        },
        ...(() => {
          const list: OutputStreamJob[] = [];
      ...
      parameters: {
        ...
        list: [
          ...
          { type: "separator", parameters: { height: 1 } },
          {
            type: "text",
            parameters: {
              text: "Hey there, stranger! Welcome to DEVUE - a fake desktop environment that I've designed and built from scratch just for fun! Please keep in mind that it's currently in beta, so some features might not work perfectly yet.",
            },
          },
          { type: "line", parameters: { height: 1 } },
          {
            type: "text",
            parameters: {
              text: "The communication between the terminal app and the DE is currently limited, but more features will be added in the future! Thanks for stopping by!",
            },
          },
          { type: "line", parameters: { height: 1 } },
...

          {
            type: "text",
            parameters: {
              text: "To navigate the site, you can either type commands into the console or click on the highlighted elements.",
            },
          },
          { type: "line", parameters: { height: 1 } },
          ...
          { type: "separator", parameters: { height: 1 } },
          {
            type: "text",
            parameters: {
              text: "Designed & Built by Gengyuan Huang © 2023",
              class: "highlight",
            },
          },
          //   {
          ...
          //     parameters: {
          //       text:
          //         '<span class="highlight">Website last updated on ' +
          //         SYSTEM_VERSION,
          //     },
          //   },
    ...
    param.outStream!.print({
      ...
      parameters: {
        ...
        list: [
          ...
          {
            type: "text",
            parameters: { text: "To know more about this website: " },
          },
          { type: "line", parameters: { height: 1 } },
          {
            type: "text",
            parameters: {
              text: "- <a target='_blank' class='clickable focus' href='https://github.com/aghnu/aghnu.me' >https://github.com/aghnu/aghnu.me</a>",
            },
          },
          {
            type: "text",
            parameters: {
              text: "- <a target='_blank' class='clickable focus' href='https://github.com/aghnu/aghnus_console' >https://github.com/aghnu/aghnus_console</a>",
            },
          },
          { type: "line", parameters: { height: 1 } },
          {
            type: "text",
            parameters: { text: "Designed & Built by Gengyuan Huang" },
          },
          { type: "line", parameters: { height: 1 } },
          { type: "separator", parameters: { height: 1 } },
          {
  ...
  execute(inputCMD: string, param: ProgramExeParam = {}) {
    ...
    if (!program_lock.locked) {
      if (cmd === "") {
        ...
        if (inputCMD !== "") {
          this.outStream.print({
            ...
            parameters: {
              text:
                "<span class='wrap'>>&nbsp</span>" +
                "<span class='wrap'>" +
                inputCMD.split(" ").join("&nbsp") +
                "</span>",
            },
          });
        }
        if (this.path[cmd] === undefined) {
          param.outStream.print({
            type: "text",
            parameters: {
              text: "<span class='highlight'>[Command Not Found]</span>",
            },
          });
        } else {
          param.outStream.print({
            type: "text",
            parameters: { text: `<span class='highlight'>[${cmd}]</span>` },
          });
      ...
      if (program_lock.input_func === null) {
        param.outStream.print({
          type: "text",
          parameters: { text: program_lock.message },
        });
      } else {
